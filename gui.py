...
#     )
    ...
    #from utils import monitor_lcd
    #threading.Thread(target=monitor_lcd, args=(folder,), daemon=True).start()

    # Reproduz batidas e energia (comandos de voz)
    from utils import reproduzir_com_eventos
    reproduzir_com_eventos(folder, mp3_path, events, meu_serial)

...
API_KEY = "b0247d1195078eed69b0c30383d65ae3"
...
def callback():
    ...
    if audio_url:
        local_filename = "musica_recebida.mp3"
        print(f"Baixando o MP3 para {local_filename}...")
        r = requests.get(audio_url)
        with open(local_filename, "wb") as f:
            f.write(r.content)
        print("Download concluído.")
        timestamp_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        dated_filename = f"musica_{timestamp_str}.mp3"

        with open(dated_filename, "wb") as f:
            f.write(r.content)
        print(f"Cópia salva como {dated_filename}.")
        
    else:
        print("Nenhum audio_url no callback!")
